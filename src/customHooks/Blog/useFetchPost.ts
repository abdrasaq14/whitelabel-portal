...
const useBlogPosts = () => {
  ...
  return {
    ...
    allPosts: blogSlice.counts.total,
   
    posts,
   
    total,
   
    totalPages: blogSlice.totalPages,
   
    setTotal,
   
    currentPage,
   
    totalDrafts: blogSlice.counts.draft,
   
    totalPublished: blogSlice.counts.published,
   
    loading: blogSlice.loading,
