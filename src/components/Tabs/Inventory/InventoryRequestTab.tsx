import React, { useEffect } from 'react'
import NoDataFound from '../../feedbacks/NoDataFound'
import Image from 'next/image'
import Spinner from '../../feedbacks/Spinner';
import { SpinnerType } from '@/enums/ComponentEnums';
import Table from '../../layouts/Table';
import Pagination from '../../feedbacks/Pagination';
import { formatDate, intlFormat } from '@/utilities/helpers';

const InventoryRequestTab = ({loading, inventoryRequestResult, handleFetchInventoryRequest, handleOpenModal}: any) => {

    // console.log("inventory requests", inventoryRequestResult)

    useEffect(() => {

        if(!inventoryRequestResult) handleFetchInventoryRequest()
    
    }, [])

    const columns = [
        { key: 'sn', label: 'S/N' },
        { key: 'requesterName', label: 'Request from'},
        { key: 'items', label: 'No of Items', render: (row: any) => <span className='font-satoshiRegular text-base text-accent-main'>{`${row?.items?.length}`}</span>},
        { key: 'totalPrice', label: 'Total Price', render: (row: any) => <span className='font-satoshiRegular text-base text-accent-main'>{intlFormat(Number(`${row?.itemDetails?.reduce((sum: number, item: any) => sum + item.unitPrice, 0)}`))}</span>},
        { key: 'createdAt', label: 'Date Requested', render: (row: any) => <span>{formatDate(row.createdAt)}</span>}
    ];

    const additionalActions = (row: any) => [
        { label: 'View Details', action: () => {handleOpenModal(row)} }
    ];
    
    return (
        <div>
            {
                loading ? 
                <div className='flex justify-center items-center h-full w-full'>
                    <Spinner type={SpinnerType.PRIMARY} height={40} width={40} />
                </div> : 
                !inventoryRequestResult ? <NoDataFound image={<Image src="/icons/nohistory.svg" alt="Crane icon" height={300} width={300} />} text="Pending inventory requests will appear here. Approve or decline requests to get started." /> :
                <>
                    <Table
                        columns={columns}
                        data={inventoryRequestResult?.results}
                        additionalActions={additionalActions}
                    />
                    <Pagination
                        page={inventoryRequestResult.page}
                        totalPages={inventoryRequestResult.totalPages}
                        onPageChange={() => {}}
                    />
                </>
            }
        </div>
    )
}

export default InventoryRequestTab