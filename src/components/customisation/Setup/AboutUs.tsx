...
const errorMessages = {
  aboutUs: { error: false, message: "About us is required" },
  aboutUsFull: { error: false, message: "About us is required" }
};
...
export default function AboutUs({ data, setAboutData, setFormError }: any) {
    ...
    if (aboutUs === null) {
      ...
      setError({
        ...
        aboutUs: {
          error:
            inputValue === "" ||
            !(inputValue.length > 50 && inputValue.length <= 300)
              ? true
              : false,
    ...
    if (aboutUsFull === null) {
      ...
      setError({
        ...
        aboutUsFull: {
          error:
            inputValue === "" ||
            !(inputValue.length > 200 && inputValue.length <= 2000)
              ? true
              : false,
          message: "About us is required"
        }
  ...
  const handleSetAboutData = (e: any) => {
    ...
    setError({
      ...
      aboutUs: {
        error:
          value === "" || !(value.length > 50 && value.length <= 300)
            ? true
            : false,
        message: "About us is required"
      }
    ...
    setFormError({
      aboutUs: value === "" || !(value.length > 50 && value.length <= 300)
    });
  };
  ...
  const handleSetFullAboutData = (e: any) => {
    ...
    setError({
      ...
      aboutUsFull: {
        error:
          value === "" || !(value.length > 200 && value.length <= 2000)
            ? true
            : false,
        message: "About us is required"
      }
    ...
    setFormError({
      aboutUsFull: value === "" || !(value.length > 200 && value.length <= 2000)
    });
  };
      ...
      <div className="border border-[#E2DFDF] mt-4 p-3">
          ...
          <div className="flex justify-between items-center">
            {error.aboutUs.error && (
              <small className="text-[#cc0000] leading-4 font-satoshiMedium text-[12px]">
                {error.aboutUs.message}
              </small>
            )}
            <small className="text-[#4D5154] leading-4 font-satoshi text-[12px] text-end tracking-tighter">
        ...
        <div className="flex flex-col gap-2 mt-5 w-full col-span-2">
          ...
          <textarea
            ...
            className="outline-none border border-[#D0D5DD] rounded-md px-[14px]"
          />
          {error.aboutUsFull.error && (
            <small className="text-[#cc0000] leading-4 font-satoshiMedium text-[12px]">
              {error.aboutUsFull.message}
            </small>
          )}
          <div className="flex items-center justify-between">
            <small className="text-[#4D5154] leading-4 font-satoshiMedium text-[10px]">
              The information provided here would be on the About Us page
            </small>
            <small className="text-[#4D5154] text-end leading-4 font-satoshi text-[12px] tracking-tighter">
