...
export default function SetupHeader({ stage, prev, isBlogChosen }: any) {
  const arrayOf3: number[] = Array.from(
    { length: isBlogChosen ? 4 : 3 },
    (_, i) => i
  );
  return (
    <div className="flex flex-col mt-10">
      ...
      <h4 className="text-[#2B2C34] font-satoshiBold text-[36px] leading-10 tracking-tighter">
        Customise your Account
      </h4>
      {stage === 0 && (
        <h6 className="mt-3 text-[#2B2C34] font-satoshiMedium text-[24px] leading-8 tracking-tighter">
          Let's get started!
        </h6>
      )}
        ...
        {arrayOf3.map((num) => (
          <div
            ...
            className={`h-[6px] rounded ${
              num === stage ? "bg-primary w-[63px]" : "bg-[#4B00821F] w-[12px]"
            }`}
          ></div>
        ))}
      </div>
