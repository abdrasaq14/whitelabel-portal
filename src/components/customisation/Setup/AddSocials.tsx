...
export default function AddSocials({ data, setSocial }: AddSocialsProps) {
  ...
  const chooseSocial = (social: string) => {
    ...
    const socialIndex = initialSelectedSocials.findIndex(
      (s: any) => s === social
    );
    if (socialIndex === -1) {
    ...
    <>
      <div className="border border-[#E2DFDF] mt-4 p-3">
        <div className="flex justify-between items-center">
          ...
          <img
            src="/icons/add_socials.svg"
            className="cursor-pointer"
            onClick={() => setSelectSocial(!selectSocial)}
            alt="add_socials_icon"
          />
        </div>
        {chosenSocials.map((social, index: number) => (
          <div key={index} className="flex flex-col gap-2 mt-3 w-full">
      ...
      <AppModal
        isOpen={selectSocial}
        closeClicked={() => setSelectSocial(!selectSocial)}
        hasClose={true}
      >
