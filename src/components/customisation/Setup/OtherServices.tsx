...
export default function OtherServices({ setService, data }: any) {
  ...
  const chooseService = (service: any) => {
    ...
    const serviceIndex = initialSelectedService.findIndex(
      (s: any) => s === service
    );
    if (serviceIndex === -1) {
  ...
  const removeService = (serviceName: string) => {
    const initialSelectedService = [...chosenService];
    const filtered = initialSelectedService.filter(
  ...
  return (
    <>
      <div className="border border-[#E2DFDF] mt-4 p-3">
        ...
        <div className="flex flex-col mt-3">
          ...
          <div onClick={() => setSelectService(!selectService)}>
            <div className="text-[14px] text-[#C8CCD0] font-satoshiRegular cursor-pointer rounded-md border border-[#C8CCD0] px-5 h-[48px] flex justify-between items-center">
              <span>Select Services</span> <BiSolidDownArrow color="#000000" />
            </div>
          </div>
          <div className="flex gap-2 mt-2">
            {chosenService.map((service: any, index: number) => (
              <div
                key={index}
      ...
      <AppModal
        ...
        closeClicked={() => setSelectService(!selectService)}
        hasClose={true}
      >
