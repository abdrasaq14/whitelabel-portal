...
const errorMessages = {
  email: { error: false, message: "Add a valid support email" },
  phone: { error: false, message: "Add a valid mobile (Ex: +1488384)" },
  noreply_email: { error: false, message: "Add a valid no-reply email" }
};
...
export default function OtherInformation({
  ...
  useEffect(() => {
    ...
    if (contactEmail === null) {
      ...
      setError({
        ...error,
        email: {
          error: inputValue === "" || !validateEmail(inputValue) ? true : false,
          message: "Add a valid support email"
        }
      });
      console.log(
        "Inner error",
      ...
      setFormError({
        contactEmail: inputValue === "" || !validateEmail(inputValue)
      });
    }
    ...
    if (senderEmail === null) {
      ...
      setError({
        ...
        noreply_email: {
          error: inputValue === "" || !validateEmail(inputValue) ? true : false,
          message: "Add a valid no-reply email"
        }
      });
      console.log(
        "Inner error",
      ...
      setFormError({
        senderEmail: inputValue === "" || !validateEmail(inputValue)
      });
    }
    ...
    if (contactPhone === null) {
      ...
      setError({
        ...error,
        phone: {
          error: inputValue === "" || !validatePhone(inputValue) ? true : false,
          message: "Add a valid mobile (Ex: +1488384)"
        }
      });
      console.log(
        "Inner error",
      ...
      setFormError({
        contactPhone: inputValue === "" || !validatePhone(inputValue)
      });
    }
  }, []);
  ...
  const handleSetContactEmail = (e: any) => {
    ...
    setError({
      ...error,
      email: {
        error: value === "" || !validateEmail(value) ? true : false,
        message: "Add a valid support email"
      }
    });
  ...
  const handleSetContactPhone = (e: any) => {
    ...
    setError({
      ...error,
      phone: {
        error: value === "" || !validatePhone(value) ? true : false,
        message: "Add a valid mobile (Ex: +1488384)"
      }
    });
  ...
  return (
    <>
      <div className="border border-[#E2DFDF] mt-4 p-3">
        <p className="font-satoshiMedium text-[16px] leading-6 tracking-tighter text-[#0E0C01]">
          Other Information
        </p>
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col gap-2 mt-3 w-full">
            <p className="font-satoshiMedium text-[14px] leading-5 tracking-tighter text-[#000000]">
              Customer Email (Customer Support)
            </p>
            <input
              name="email"
              type="email"
            ...
            />
            {error.email.error && (
              <small className="text-[#cc0000] leading-4 font-satoshiMedium text-[12px]">
                {error.email.message}
              </small>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 mt-3 w-full">
            ...
            <div className="flex items-center w-full">
              <div
                className="h-[40px] bg-primary px-2 rounded-l-md flex justify-center items-center cursor-pointer"
                onClick={() => setOpenCountryCodes(true)}
              >
                <span className="text-[#ffffff] text-[16px] leading-6">
                  +{contact.phone.cCode || chosenCountryCode}
                </span>
          ...
          <div className="flex flex-col gap-2 mt-3 w-full col-span-2">
            ...
            <input
              ...
              className="outline-none border border-[#D0D5DD] rounded-md h-[40px] px-[14px]"
            />
            {error.noreply_email.error && (
              <small className="text-[#cc0000] leading-4 font-satoshiMedium text-[12px]">
                {error.noreply_email.message}
              </small>
            )}
            <small className="text-[#4D5154] leading-4 font-satoshiMedium text-[12px] tracking-tighter">
              This email would be used to send notifications to users on your
              platform
            </small>
          </div>
        </div>
      ...
      <AppModal
        isOpen={openCountryCodes}
        closeClicked={() => setOpenCountryCodes(!openCountryCodes)}
        hasClose={true}
      >
        <div className="flex flex-col px-5 w-full">
          <h2 className="font-satoshiBold text-[20px] leading-7 text-[#000000] tracking-tighter">
            Select Country Code
          </h2>
              ...
              <div key={index} className="flex items-center">
                <p
                  onClick={() => {
                    ...
                    setOpenCountryCodes(!openCountryCodes);
                  }}
                  className="cursor-pointer hover:text-[#4B0082] hover:font-satoshi text-[#000000] font-satoshiRegular text-[16px] leading-6"
                >
