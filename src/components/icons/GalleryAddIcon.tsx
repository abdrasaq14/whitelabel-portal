import React from 'react'

const GalleryAddIcon = () => {
  return (
    <svg 
        width="20" 
        height="20" 
        viewBox="0 0 20 20" 
        fill="none" 
        xmlns="http://www.w3.org/2000/svg"
    >
        <path 
            d="M7.4987 8.33333C8.41917 8.33333 9.16536 7.58714 9.16536 6.66667C9.16536 5.74619 8.41917 5 7.4987 5C6.57822 5 5.83203 5.74619 5.83203 6.66667C5.83203 7.58714 6.57822 8.33333 7.4987 8.33333Z" 
            fill="#2B2C34" 
            stroke="#292D32" 
            strokeWidth="1.5" 
            strokeLinecap="round" 
            strokeLinejoin="round"
        />
        <path 
            d="M10.8346 1.66602H7.5013C3.33464 1.66602 1.66797 3.33268 1.66797 7.49935V12.4993C1.66797 16.666 3.33464 18.3327 7.5013 18.3327H12.5013C16.668 18.3327 18.3346 16.666 18.3346 12.4993V8.33268" 
            stroke="#292D32" 
            strokeWidth="1.5" 
            strokeLinecap="round" 
            strokeLinejoin="round"
        />
        <path 
            d="M13.125 4.16602H17.7083" 
            stroke="#292D32" 
            strokeWidth="1.5" 
            strokeLinecap="round"
        />
        <path 
            d="M15.418 6.45833V1.875" 
            stroke="#292D32" 
            strokeWidth="1.5" 
            strokeLinecap="round"
        />
        <path 
            d="M2.22656 15.7918L6.3349 13.0335C6.99323 12.5918 7.94323 12.6418 8.5349 13.1501L8.8099 13.3918C9.4599 13.9501 10.5099 13.9501 11.1599 13.3918L14.6266 10.4168C15.2766 9.85846 16.3266 9.85846 16.9766 10.4168L18.3349 11.5835" 
            stroke="#292D32" 
            strokeWidth="1.5" 
            strokeLinecap="round" 
            strokeLinejoin="round"
        />
    </svg>
  )
}

export default GalleryAddIcon